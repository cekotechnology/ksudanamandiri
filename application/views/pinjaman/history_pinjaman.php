...
<?php
	...
	if ($dataPinjaman->num_rows() > 0) {
		...
		if ($adaTunggakan > 0) { //Ada Tunggakan sebelum Februari 2019
			...
			$idc = 3;
			$data ='+'.$add.' month';
			$date =date('m', strtotime($dPj->tgl_pengajuan));
			$tgl =date('d', strtotime($dPj->tgl_pengajuan));
			if($date =date('m','2') && $tgl > date('d','27')){
				$data ='+28 days';
			}
			...
			echo "
				...
				<td class='nomor'>{$table_counter}.</td>
				<td class='tgl_byr'>".date('d-m-Y', strtotime($data, strtotime($dPj->tgl_pengajuan))) ."	
				</td>
