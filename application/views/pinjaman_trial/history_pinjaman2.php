...
<h4>History Pinjaman Anggota TRIAL 2</h4>
<p>&nbsp;</p>
...
//   $adaTunggakan = 0;
   ...
   $dataAngsuran = $this->db->query("SELECT * FROM pinjaman_detail_trial WHERE username = '$user_session'
   ...
   if ($dataAngsuran->num_rows() > 0) { //Ada Angsuran
    ...
    foreach ($dataAngsuran->result_array() as $key => $a) {
     ...
     $getAngsPeriode = $this->db->query("SELECT * FROM pinjaman_detail_trial
     ...
     if ($tglSkr == $lJatuhTempo) {
      ...
      $cekAngsBlnSkr = $this->db->query("SELECT * FROM pinjaman_detail_trial
      WHERE username ='$user_session' AND id_pinjam = '$dPj->id_pinjam'
    ...
    $tglJt        = "01-" . date('m-Y', strtotime('2019-02-01'));
