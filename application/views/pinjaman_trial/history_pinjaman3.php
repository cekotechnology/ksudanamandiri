...
//   debug($dataPinjaman->result_array());
   ...
   foreach ($dataPinjaman->result() as $key => $value) {
    ...
    $dataAngsuran = $this->db->query("SELECT * FROM pinjaman_detail_trial WHERE username = '$user_session' AND id_pinjam = '$value->id_pinjam'  group by date_format(tgl_bayar, '%m-%Y') order by tgl_bayar");
    ...
    if ($dataAngsuran->num_rows() > 0) {
     ...
     foreach ($dataAngsuran->result_array() as $key => $r) {
      ...
      $bulan_con = date("m-Y", $date);
      $query     = $this->db->query("select * from pinjaman_detail_trial where date_format(tgl_bayar, '%m-%Y') = '" . $bulan_con . "' and id_pinjam = '" . $value->id_pinjam . "' order by tgl_bayar asc ")->result();
      ...
      if (count($query) > 0) {
       foreach ($query as $keys => $values) {
        ...
        $_angsuran = $this->db->query("select sum(angsuran) as angsuran from pinjaman_detail_trial where id_pinjam = '" . $value->id_pinjam . "'")->row();
        ...
        echo "<td style='background:#dbffdb;text-align:right;'>" . number_format($sisaPjm - $values->angsuran) . "</td>";
        echo "</tr>";
